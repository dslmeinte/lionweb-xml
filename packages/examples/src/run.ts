...
import { builtinPrimitives, isINamed, nameOf, serializeLanguages } from "@lionweb/core"
...
type RunOptions = Partial<{
    ...
    postReadAdjustment?: (ePackage: EPackage) => void
}>

const runOnce = async (filePath: string, options?: RunOptions) => {
    const path = filePath.substring(0, filePath.lastIndexOf("/"))
    const fileNameWithExtension = filePath.substring(filePath.lastIndexOf("/") + 1)
    const [fileName] = fileNameWithExtension.split(".")

    console.log(`reading Ecore file <${fileName}.ecore>:`)
...
const types = await runOnce("artifacts/sysmlv2/types.ecore", { transformer: { eDataTypesToPrimitiveTypes: true } })
...
await runOnce("artifacts/sysmlv2/kerml.ecore", {
    reader: {
